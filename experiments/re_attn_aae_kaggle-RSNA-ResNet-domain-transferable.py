...
# Paired bootstrap significance test for the headline C1 vs C4 claim (same resample
# indices applied to both score arrays each draw, so the test is on the paired difference).
c1_v_c4_sig = bootstrap_paired_diff(scores_c4, scores_c1, binary_test)
all_results['C1_vs_C4_sig'] = c1_v_c4_sig
print(f"\n  C1 vs C4 significance (paired bootstrap, n=1000):")
print(f"  ΔAUC-ROC = {c1_v_c4_sig['diff_mean']:+.4f}  "
...
disc_stable = {k: disc_stability_label(all_results[k + '_disc']) if k + '_disc' in all_results else '-'
               for k in ['C1','C2','C3','C4','C5','C6','C7']}
for k in ['C1','C2','C3','C4','C5','C6','C7']:
    r = all_results[k]
